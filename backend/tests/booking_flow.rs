...
use hms_backend::domain::models::RoomHoldType;
use hms_backend::domain::repositories::{
    AuditRepository, BookingRepository, GuestRepository, InvoiceRepository, RoomHoldRepository,
    RoomRepository,
};
...
use hms_backend::infrastructure::repository::postgres_room_hold::PostgresRoomHoldRepository;
use std::sync::Arc;
use uuid::Uuid;
...
#[sqlx::test]
async fn booking_flow_rejects_room_with_overlapping_hold(pool: sqlx::PgPool) {
    let room_repo = Arc::new(PostgresRoomRepository::new(pool.clone())) as Arc<dyn RoomRepository>;
    let booking_repo =
        Arc::new(PostgresBookingRepository::new(pool.clone())) as Arc<dyn BookingRepository>;
    let audit_repo =
    ...
    let room = room_service
        .create_room(hotel_id, "A103".to_string(), "Suite".to_string(), 12000_i64)
        .await
        .unwrap();

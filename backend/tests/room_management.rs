...
async fn room_service_creates_room(pool: sqlx::PgPool) {
    ...
    let room = service
        .create_room(hotel_id, room_number.clone(), room_type.clone(), price_cents)
        .await
        .unwrap();
    ...
    let found = service.room_repo.find_by_id(hotel_id, room.id).await.unwrap().unwrap();
...
async fn room_service_enforces_transitions(pool: sqlx::PgPool) {
    ...
    service.update_room_status(hotel_id, room.id, hms_backend::domain::models::RoomStatus::Occupied).await.unwrap();

    // Occupied -> Available: FAIL (must be Dirty first)
    let result = service.update_room_status(hotel_id, room.id, hms_backend::domain::models::RoomStatus::Available).await;
    assert!(result.is_err());
    
    // Occupied -> Dirty: OK
    service.update_room_status(hotel_id, room.id, hms_backend::domain::models::RoomStatus::Dirty).await.unwrap();
}

...
const HousekeepingPage = () => {
  ...
  return <div className="space-y-5">
    ...
    <SectionCard className="space-y-4">
      ...
      {query.isLoading ? <div className="space-y-2" aria-label="Cargando turno"><div className="h-16 animate-pulse rounded-xl bg-muted" /><div className="h-16 animate-pulse rounded-xl bg-muted" /></div> : null}
      {!query.isLoading && !query.error && visibleQueue.length === 0 ? <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">{search ? "Ninguna habitación coincide con la búsqueda." : filter === "shift" ? "El turno está bajo control" : "No hay casos en este filtro."}</div> : null}
      {!query.isLoading && !query.error && visibleQueue.length > 0 ? <div className="grid min-h-[520px] gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(360px,42%)]"><ol className="max-h-[calc(100vh-22rem)] space-y-2 overflow-y-auto" aria-label="Cola de Housekeeping">{visibleQueue.map((item) => <li key={item.room_id}><button type="button" aria-label={`Ver tarea habitación ${item.room_number}`} aria-current={selected?.room_id === item.room_id ? "true" : undefined} onClick={() => setSelectedRoomId(item.room_id)} className={`min-h-[76px] w-full rounded-xl border p-4 text-left ${selected?.room_id === item.room_id ? "border-primary bg-primary/5 ring-2 ring-primary/20" : "border-border bg-card"}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black">Habitación {item.room_number} · {item.room_type}</p><p className="mt-1 text-xs font-semibold text-muted-foreground">{item.room_status === "Dirty" ? "Por limpiar" : item.room_status === "Cleaning" ? "En limpieza" : item.room_status === "Available" ? "Lista" : "Mantenimiento"}{item.turnover_today ? " · Salida de hoy" : ""}</p>{item.departure ? <p className="mt-1 text-xs text-foreground">Huésped: {item.departure.guest_name}</p> : null}</div><span className="rounded-full border border-border px-2 py-1 text-[10px] font-bold uppercase">{item.isBlocked ? "Atención" : "Turno"}</span></div></button></li>)}</ol><SectionCard className="min-h-[520px] overflow-hidden p-0"><HousekeepingRoomWorkspace item={selected} canWrite={canWrite} loadingAction={loadingAction} onClose={closeRoom} onStart={(id) => void run(`${id}:start`, () => startCleaning(id), "Limpieza iniciada", "start_cleaning")} onFinish={(id) => void run(`${id}:finish`, () => finishCleaning(id), "Limpieza finalizada", "finish_cleaning")} onOpenMaintenance={(id, payload) => void run(`${id}:maintenance`, () => sendRoomToMaintenance(id, payload), "Incidencia abierta", "handle_blocker")} onResolveMaintenance={(id, payload) => void run(`${id}:resolve`, () => returnRoomToDirty(id, payload), "Mantenimiento resuelto", "handle_blocker")} /></SectionCard></div> : null}
    </SectionCard>
  </div>;
};

...
const HousekeepingRoomWorkspace = ({ item, canWrite, loadingAction, onStart, onFinish, onOpenMaintenance, onResolveMaintenance, onClose }: Props) => {
  ...
  return <div className="fixed inset-x-0 bottom-0 z-30 flex max-h-[86vh] min-h-[min(520px,86vh)] flex-col rounded-t-2xl border border-border bg-card shadow-2xl xl:static xl:z-auto xl:max-h-none xl:min-h-0 xl:rounded-none xl:border-0 xl:shadow-none">
    <div className="border-b border-border px-5 py-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Detalle de tarea</p><h2 className="mt-1 text-xl font-black">Habitación {item.room_number}</h2><p className="mt-1 text-sm text-muted-foreground">{item.room_type} · {status}{item.turnover_today ? " · Salida de hoy" : ""}</p></div><button type="button" aria-label="Cerrar detalle" className="min-h-11 min-w-11 rounded-xl border border-border xl:hidden" onClick={onClose}><X className="mx-auto h-4 w-4" /></button></div></div>

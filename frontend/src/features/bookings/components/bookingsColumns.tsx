  ...
  },
  {
    ...
    cell: (item) => (
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="sm" className="text-xs font-bold text-indigo-600" onClick={() => onViewDetails(item)}>
          Detalles
        </Button>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            {item.status !== "Cancelled" && item.status !== "CheckedOut" ? (
              <DropdownMenuItem className="text-red-600" onClick={() => onCancelBooking(item.id)}>
                Cancelar Reserva
              </DropdownMenuItem>
            ) : null}
          </DropdownMenuContent>

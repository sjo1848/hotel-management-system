...
const AlertCard = ({ alert, type, onOpenBooking }: AlertCardProps) => (
  <div
    className="group flex cursor-pointer items-center justify-between rounded-xl border border-slate-100 bg-slate-50 p-4 transition-all hover:bg-white hover:shadow-md"
  ...
  >
    <div className="flex items-center gap-4">
      ...
      <div>
        <div className="text-sm font-black tracking-tight text-slate-900">{alert.guest_name}</div>
        <div className="mt-0.5 text-[10px] font-bold uppercase tracking-widest text-slate-400">Habitación {alert.room_number}</div>
      </div>
    </div>
...
const DashboardAlertsPanel = ({ loading, kpis, onOpenBooking }: DashboardAlertsPanelProps) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-black tracking-tight text-slate-900">Alertas de Hoy</h3>
        <p className="mt-1 text-xs font-bold uppercase tracking-widest text-slate-400">Llegadas y Salidas</p>
      </div>

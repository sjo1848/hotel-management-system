  ...
  >
    <div className="flex items-center gap-4">
      ...
      <div>
        ...
        <div className="mt-0.5 text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">Habitación {alert.room_number}</div>
      </div>
    </div>
...
const renderAlertGroup = (
  ...
  <div className="space-y-2">
    <span className="ml-1 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">
      {title} ({count})
    </span>
    {loading ? (
      Array.from({ length: 2 }).map((_, i) => <Skeleton key={`${title}-sk-${i}`} className="h-16 w-full rounded-xl" />)
    ) : alerts.length === 0 ? (
      <div className="rounded-xl border border-dashed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/70 p-4 text-center text-xs font-bold text-slate-500 dark:text-slate-400">
        {emptyLabel}
      </div>
    ) : (
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">Llegadas y Salidas</p>
      </div>

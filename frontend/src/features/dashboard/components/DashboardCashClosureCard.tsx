...
const DashboardCashClosureCard = ({ balance, isClosing, onCloseCash }: DashboardCashClosureCardProps) => (
  <Card className="group relative h-full overflow-hidden rounded-3xl border-none bg-slate-900 text-white shadow-2xl shadow-slate-200/60">
    ...
    <CardHeader>
      ...
      <p className="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Balance del Turno Actual</p>
    </CardHeader>
    <CardContent className="relative z-10 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 border-y border-white/10 py-4">
        <div>
          <p className="text-[10px] font-bold uppercase text-slate-400 dark:text-slate-500">Efectivo</p>
          <p className="text-lg font-bold text-white">${((balance?.cash_amount_cents ?? 0) / 100).toLocaleString()}</p>
        </div>
        <div>
          <p className="text-[10px] font-bold uppercase text-slate-400 dark:text-slate-500">Tarjeta</p>
      ...
      <Button
        ...
        className="h-12 w-full rounded-xl bg-white dark:bg-slate-900 text-[10px] font-black uppercase tracking-widest text-slate-900 dark:text-slate-100 shadow-xl hover:bg-slate-100 dark:hover:bg-slate-700"
      >

  ...
  const columns: Column<Room>[] = [
    {
      ...
      cell: (item) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-medium text-slate-900 dark:text-slate-100">Habitación {item.room_number}</div>
            <div className="text-xs text-slate-500 dark:text-slate-400">{item.room_type}</div>
          </div>
        </div>
      ),
    },
    ...
    {
      header: "Precio / Noche",
      cell: (item) => <span className="font-mono text-slate-700 dark:text-slate-300">${(item.price_cents / 100).toLocaleString()}</span>,
    },
    {
      header: "Estado",
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {rooms.length === 0 && !loading && (
        <div className="flex flex-col items-center justify-center py-20 bg-slate-50 dark:bg-slate-800/70 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl text-slate-500 dark:text-slate-400 animate-in fade-in zoom-in duration-500">
          ...
          <h3 className="text-lg font-bold text-slate-700 dark:text-slate-300">No hay habitaciones disponibles</h3>
        ...
        viewMode === "list" ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {rooms.map((room) => (
              <div
                key={room.id}
                className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group relative overflow-hidden"
              >
                <div className={cn(
      ...
      {loading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-slate-50 dark:bg-slate-800/70 rounded-xl border border-slate-100 dark:border-slate-800 p-5 h-[180px] animate-pulse" />
          ))}
        </div>
      )}
